    ...
    public void run() {
      try {
        while (mustRedoHighlighter.get()) {
          ...
          document.setCharacterAttributes(0, document.getLength(), defaultSet,
              false);
          document.setParagraphAttributes(0, document.getLength(), defaultSet,
              false);
          for (int it = 0; it < tokens.size(); it++) {
            ...
            if (token.type == Token.Type.STRING) {
              document.setCharacterAttributes(token.start,
                  token.source.length(), stringSet, false);
            } else if (token.type == Token.Type.NUMBER) {
              document.setCharacterAttributes(token.start,
                  token.source.length(), numberSet, false);
            } else if (token.type == Token.Type.KEYWORD) {
              document.setCharacterAttributes(token.start,
                  token.source.length(), keywordSet, false);
            } else if (token.type == Token.Type.VARIABLE) {
              document.setCharacterAttributes(token.start,
                  token.source.length(), variableSet, false);
            } else if (token.type == Token.Type.BLOCK) {
              if (token.start == blockIndex) {
                document.setCharacterAttributes(token.start,
                    token.source.length(), blockSet, false);
                final int adder = token.source.equals("(") ? 1 : -1;
                final String expected =
                  token.source.equals("(") ? ")" : "(";
                int sameFound = 0;
                for (int ib = it + adder; ib >= 0
                    && ib < tokens.size(); ib = ib + adder) {
                  if (token.source.equals(tokens.get(ib).source)) {
                    ...
                    if (sameFound == 0) {
                      ...
                      document.setCharacterAttributes(
                          tokens.get(ib).start,
                          tokens.get(ib).source.length(),
                          blockSet, false);
                      int blockStart = Math.min(token.start,
                          tokens.get(ib).start);
                      int blockEnd = Math.max(token.start,
                          tokens.get(ib).start);
                      document.setCharacterAttributes(blockStart,
                          blockEnd - blockStart + 1, underSet,
                          false);
                      break;
                    } else {
                      sameFound--;
              ...
              document.setCharacterAttributes(token.start,
                  token.source.length(), unknownSet, false);
            }
          }
        }
      } catch (Exception e) {
